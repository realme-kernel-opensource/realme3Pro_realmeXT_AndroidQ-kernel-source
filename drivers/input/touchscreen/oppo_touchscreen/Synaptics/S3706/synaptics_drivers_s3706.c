...
static void synaptics_game_mode_control(struct chip_data_s3706 *chip_info, bool enable)
{
	//addr：0x041d  open：bit2 set 1 close:bit2 set 0
	int ret = 0,arg=0;
	...
	if(enable) {
		arg = ret | 0x04;
                TPD_DEBUG("%s enable is %d, arg is 0x%x\n", __func__, enable, arg);
                touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F51_CUSTOM_CTRL13, arg);
        } else {
		arg = ret & 0xfb;
                TPD_DEBUG("%s enable is %d, arg is 0x%x\n", __func__, enable, arg);
                touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F51_CUSTOM_CTRL13, arg);
...
static int synaptics_mode_switch(void *chip_data, work_mode mode, bool flag)
{
	...
	case MODE_GAME_CONTROL:
		synaptics_game_mode_control(chip_info,flag);
		break;
	default:
